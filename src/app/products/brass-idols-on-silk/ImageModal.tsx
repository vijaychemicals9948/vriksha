...
type Props = {
    ...
    onClose: () => void;
    onMove?: (dir: 1 | -1) => void; // <- tightened to match your page
};

const MIN_SCALE = 1;
const DEFAULT_SCALE = 1;
const INITIAL_ZOOM_SCALE = 2.8;
const MAX_SCALE = 4;
const MIN_WHEEL_SCALE = 1;

export default function ImageModal({ products, open, activeIndex, onClose, onMove }: Props) {
    // ---- ALL HOOKS: declare unconditionally at top ----
    ...
    // keyboard/focus effect (always registered when component mounted)
    useEffect(() => {
    ...
    // reset/clamp when scale changes
    useEffect(() => {
        if (scale <= 1.001) {
    ...
    // ---- early return for render only (hooks already run above) ----
    ...
    function scheduleOriginUpdate(newOrigin: { x: number; y: number }) {
        pendingOrigin.current = newOrigin;
    ...
    function onMouseMove(e: React.MouseEvent) {
        if (scale === MIN_SCALE || isPanning) return;
        const p = pointToPercent(e.clientX, e.clientY);
        scheduleOriginUpdate(p);
    }
    ...
    function onWheel(e: React.WheelEvent) {
        ...
        setScale((prev) => {
            const next = Math.max(MIN_WHEEL_SCALE, Math.min(MAX_SCALE, +(prev * factor).toFixed(3)));
            return next;
        });
        const p = pointToPercent(e.clientX, e.clientY);
        scheduleOriginUpdate(p);
    ...
    function clampTranslateToBounds(tx: number, ty: number) {
        ...
        const maxOffsetX = Math.max(0, (scaledW - wrapRect.width) / 2);
        const maxOffsetY = Math.max(0, (scaledH - wrapRect.height) / 2);
    ...
    return (
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={product.title ?? "Image modal"}
        >
            <button aria-label="Close" className={styles.closeBtn} onClick={onClose} ref={closeBtnRef}>
                ×
            </button>
            ...
            <div className={styles.zoomWrap}>
                <div
                    ...
                    style={{ cursor: scale === MIN_SCALE ? "zoom-in" : isPanning ? "grabbing" : "grab" }}
                ...
                <div className={styles.caption}>
                    {product.title}
                    <span className={styles.hint}> — Click/tap to zoom, wheel to zoom, drag to pan</span>
                </div>
            </div>
        </div>
    );
